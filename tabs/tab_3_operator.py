...
def render_tab2(selected_customer, selected_plant, reactor_vol, gas_stripping_factor, selected_lang="Nederlands", is_admin=True, temp_regime="Mesofiel (~38°C)"):
    ...
    if auto_balance_clicked:
        ...
        new_recirc_drijfmest = min(70.0, max(30.0, 40.0 + extra_dry_load))
        
        st.session_state["input_recirc_Drijfmest (rund)"] = round(new_recirc_drijfmest, 1)

        st.success(f"⚖️ Auto-Balance doorgevoerd! Recirculatie op drijfmest aangepast naar {st.session_state['input_recirc_Drijfmest (rund)']}% om het verhoogde DM-gehalte te compenseren.")
        st.rerun()
    ...
    dm_alert = avg_dm_system > 12.0 or avg_dm_system < 6.0
    
    if olr_alert or vzv_alert or dm_alert:
        reasons = []
        ...
        st.error(f"🔴 **Proces Alert:** " + " | ".join(reasons) + ". Klik op Auto-Balance om automatisch bij te sturen.")
    else:
    ...
    with m4:
        st.metric("Benodigd IJzer Product", f"{product_needed_kg:.1f} kg/d", delta=f"{int(bags_needed_day)} zakken")
